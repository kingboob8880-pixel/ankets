...
(function () {
  ...
  function sanitizeDownloadFileName(name) {
    return String(name || 'export.json').replace(/[<>:"/\\|?*\u0000-\u001f]/g, '_');
  }

  /**
   * Сохранение текста как файла (blob + a[download]).
   * application/octet-stream чаще триггерит «Сохранить» в Safari/iOS, чем application/json.
   */
  function downloadTextAsFile(text, fileName) {
    ...
    a.style.cssText = 'position:fixed;left:0;top:0;width:1px;height:1px;opacity:0.001;';
    ...
    function render() {
      ...
      if (isFinal) {
        ...
        row.appendChild(el('button', { type: 'button', class: 'btn btn-ghost', text: u.downloadJson, onclick: () => {
          const name = `rukya-questionnaire-${externalId}.json`;
          try {
            downloadTextAsFile(jsonStr, name);
            setStatus(u.fileSaved);
          } catch (e) {
            console.warn('download', e);
            setStatus(u.downloadFail);
          }
        } }));
